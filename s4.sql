...
SELECT uit.cursus, 
       uit.begindatum,
       COUNT(ins.cursist) AS aantal_inschrijvingen
FROM uitvoeringen uit
JOIN inschrijvingen ins 
ON uit.cursus = ins.cursus AND uit.begindatum = ins.begindatum
WHERE uit.begindatum >= '2019-01-01' AND uit.begindatum < '2020-01-01'
GROUP BY uit.cursus, uit.begindatum
HAVING COUNT(ins.cursist) >= 3;
...
SELECT DISTINCT i.cursist AS medewerkernummer, i.cursus AS cursuscode
FROM inschrijvingen i
WHERE EXISTS (
    SELECT 1
    FROM inschrijvingen
    WHERE cursist = i.cursist AND cursus = i.cursus
...
SELECT DISTINCT u.cursus, 
       (SELECT COUNT(*)
        FROM uitvoeringen
        WHERE cursus = u.cursus) AS aantal
FROM uitvoeringen u
ORDER BY u.cursus;
-- [TEST]
